modality = 'j'
graph = 'smpl'
work_dir = f'./work_dirs/dgstgcn/aist++_smpl_360/{modality}_aug'
...
model = dict(
    ...
    backbone=dict(
        ...
        graph_cfg=dict(layout=graph, mode='random', num_filter=16, init_off=.04, init_std=.02),
        num_person=1,
        base_channels=128,
        ch_ratio=1.5,
        inflate_stages=[3, 6],
...
ann_file = 'data/aist++/aist++_smpl_360.pkl'
train_pipeline = [
    ...
    dict(type='UniformSampleDecode', clip_len=350),
...
val_pipeline = [
    dict(type='PreNormalize3D', align_spine=False),
    dict(type='GenSkeFeat', dataset=graph, feats=[modality]),
    dict(type='UniformSampleDecode', clip_len=350, num_clips=1),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
]
test_pipeline = [
    dict(type='PreNormalize3D', align_spine=False),
    dict(type='GenSkeFeat', dataset=graph, feats=[modality]),
    dict(type='UniformSampleDecode', clip_len=350, num_clips=3),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
]
