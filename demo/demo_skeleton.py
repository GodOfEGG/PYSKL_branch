...
def main():
    ...
    # for unknown reason, resolution of each frame cannot be odd number
    vis_frames = [cv2.resize(x, (x.shape[1]//2*2, x.shape[0]//2*2)) for x in vis_frames]
    # bgr to rgb
    vis_frames = [cv2.cvtColor(x, cv2.COLOR_BGR2RGB) for x in vis_frames]
